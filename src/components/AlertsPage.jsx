...
const allAlertsKL = [
    {
        ...
        desc: 'Road construction works ahead. Buses rerouted via Chittoor Road and Banerji Road until further notice.',
    ...
    },
    {
        ...
        desc: 'All buses running on standard timetable with normal frequencies. No disruptions reported between Vyttila Hub and Aluva.',
    ...
    },
    {
        ...
        desc: 'Traffic congestion near Edappally Bypass Junction causing a 12-minute delay. Alternate route via Seaport-Airport Road advised.',
    ...
    },
    {
        ...
        type: 'new-route',
        severity: 'info',
        title: 'New SWIFT Electric Route KL-2E Launched',
        titleKey: 'newSwiftElectricTitle',
        desc: 'New express SWIFT electric bus KL-2E operational from Kakkanad Infopark. Reduced travel time by 35% on the IT corridor.',
    ...
    },
    {
        ...
        type: 'diversion',
        severity: 'warning',
        title: 'Vyttila Overpass Closure',
        titleKey: 'vyttilaOverpassClosureTitle',
        desc: 'Overpass closed for load testing inspection. Buses on routes KL-5, KL-6, KL-7 diverted via SA Road.',
        descKey: 'vyttilaOverpassClosureDesc',
        route: 'KL-5, KL-6, KL-7',
        time: '2 hours ago',
        timeKey: 'hoursAgo',
        timeVal: '2',
        icon: AlertTriangle,
    ...
    },
    {
        ...
        type: 'delay',
        severity: 'warning',
        title: 'Route KL-2 — Major Delay 18 min',
        titleKey: 'routeKL2MajorDelayTitle',
        desc: 'Accident near Kakkanad junction causing significant traffic backup. Expect extended wait times.',
        descKey: 'routeKL2MajorDelayDesc',
        route: 'KL-2',
        time: '3 hours ago',
        timeKey: 'hoursAgo',
        timeVal: '3',
    ...
    },
    {
        ...
        type: 'normal',
        severity: 'success',
        title: 'Route KL-3 — Resumed Normal Service',
        titleKey: 'routeKL3NormalTitle',
        desc: 'Earlier diversion lifted. Route KL-3 has resumed its normal path through Ernakulam South and Fort Kochi.',
        descKey: 'routeKL3NormalDesc',
        route: 'KL-3',
        time: '4 hours ago',
        timeKey: 'hoursAgo',
        timeVal: '4',
    ...
    },
    {
        ...
        desc: 'Real-time GPS telemetry refresh completed for Central, Kakkanad, and Coastal zones. ETA accuracy improved.',
...
const categoryLabel = (cat, lang) => {
    const map = {
        all: t('filterAll', lang) || 'All',
        delay: t('filterDelays', lang) || 'Delays',
        diversion: t('filterDiversions', lang) || 'Diversions',
        service: t('filterService', lang) || 'Service',
        system: t('filterSystem', lang) || 'System'
    };
    return map[cat] || cat;
};

const formatAlertTime = (alert, lang) => {
    if (alert.timeKey === 'minsAgo') {
        return `${alert.timeVal} ${t('minsAgo', lang) || 'mins ago'}`;
    }
    if (alert.timeKey === 'hourAgo') {
        return `${alert.timeVal} ${t('hourAgo', lang) || 'hour ago'}`;
    }
    if (alert.timeKey === 'hoursAgo') {
        return `${alert.timeVal} ${t('hoursAgo', lang) || 'hours ago'}`;
    }
    return alert.time;
};

export default function AlertsPage({ navigateTo, lang, region }) {
    const allAlerts = region === 'Kerala' ? allAlertsKL : allAlertsTN;
    ...
    return (
        <div className="max-w-5xl mx-auto w-full pb-16 pt-4 px-0 sm:px-2 transition-colors duration-300">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
                <div>
                    ...
                    <div className="flex items-center space-x-3 mb-2">
                        <div className="inline-flex items-center space-x-2 bg-red-50 dark:bg-red-950/20 px-3 py-1 rounded-full border border-red-200 dark:border-red-900/30 text-red-600 dark:text-red-400 font-extrabold text-[10px] uppercase tracking-widest">
                            ...
                            <span>{t('liveFeed', lang) || 'Live Feed'}</span>
                        </div>
                    </div>
                    <h1 className="text-3xl sm:text-4xl font-black text-[#0F1E36] dark:text-white tracking-tight mb-1">
                        {t('updatesAndAlerts', lang) || 'Updates & Alerts'}
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-bold leading-relaxed">
                        {t('updatesAndAlertsDesc', lang) || 'Real-time transit alerts, diversions, and service updates across all routes.'}
                    </p>
                </div>
                ...
                <div className="flex items-center space-x-2 shrink-0">
                    ...
                    <div className="bg-[#FF9933] text-white text-xs font-black px-3 py-2 rounded-xl shadow-sm flex items-center space-x-1.5">
                        ...
                        <span>{stats.total} {t('activeAlerts', lang) || 'Active'}</span>
                    </div>
                </div>
            </div>
                      {/* Stats Row */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
                {[
                    { labelKey: 'totalAlerts', value: stats.total, color: 'text-[#0F1E36] dark:text-white', icon: Bell, bg: 'bg-gray-50 dark:bg-[#0B1E36]', border: 'border-gray-200 dark:border-slate-800' },
                    { labelKey: 'delays', value: stats.delays, color: 'text-blue-600 dark:text-blue-400', icon: Clock, bg: 'bg-blue-50/60 dark:bg-blue-950/10', border: 'border-blue-200 dark:border-blue-900/30' },
                    { labelKey: 'diversions', value: stats.diversions, color: 'text-orange-600 dark:text-orange-400', icon: AlertTriangle, bg: 'bg-orange-50/60 dark:bg-orange-950/10', border: 'border-orange-200 dark:border-orange-900/30' },
                    { labelKey: 'serviceUpdates', value: stats.service, color: 'text-emerald-600 dark:text-emerald-400', icon: TrendingUp, bg: 'bg-emerald-50/60 dark:bg-emerald-950/10', border: 'border-emerald-200 dark:border-emerald-900/30' },
                ].map((s) => (
                    <div key={s.labelKey} className={`${s.bg} border ${s.border} rounded-2xl p-4 flex items-center space-x-3`}>
                        ...
                        <div>
                            ...
                            <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-550 block mt-0.5">{t(s.labelKey, lang)}</span>
                        </div>
                    </div>
                ))}
            </div>
                ...
                <AnimatePresence>
                    {filtered.length === 0 && (
                        ...
                        >
                            ...
                            <h3 className="text-lg font-black text-[#0F1E36] dark:text-white mb-2">{t('noAlerts', lang)}</h3>
                            <p className="text-xs text-gray-500 dark:text-gray-400 font-bold">{t('noAlertsInCategory', lang)}</p>
                        </motion.div>
                    )}

                    {filtered.map((alert, idx) => {
                        const isExpanded = expandedId === alert.id;
                        const titleText = alert.titleKey ? t(alert.titleKey, lang) : alert.title;
                        const descText = alert.descKey ? t(alert.descKey, lang) : alert.desc;
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center flex-wrap gap-2 mb-1">
                                                    ...
                                                    <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider ${alert.badgeBg} shrink-0`}>
                                                        {t(alert.category, lang) || alert.category}
                                                    </span>
                                                </div>
                                                ...
                                                {isExpanded && (
                                                    ...
                                                    >
                                                        <div className="flex items-center space-x-1.5 text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                                            ...
                                                            <span>{t('routeLabel', lang) || 'Route'}: {alert.route}</span>
                                                        </div>
                                                        <div className="flex items-center space-x-1.5 text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                                            <Clock className="w-3.5 h-3.5" />
                                                            <span>{formatAlertTime(alert, lang)}</span>
                                                        </div>
                                                        <div className="flex items-center space-x-1.5 text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                                            <MapPin className="w-3.5 h-3.5" />
                                                            <span>{t('chennaiMetroRegion', lang) || 'Chennai Metro Region'}</span>
                                                        </div>
                                                    </motion.div>
                                                )}

                                                <div className="flex items-center space-x-3 mt-2">
                                                    <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{formatAlertTime(alert, lang)}</span>
                                                    <span className="w-1 h-1 bg-gray-300 dark:bg-slate-700 rounded-full"></span>
                                                    <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{t('routeLabel', lang) || 'Route'} {alert.route}</span>
                                                </div>
                                            </div>
                                        </div>
            ...
            <div className="mt-8 bg-[#0B1B2D] rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <span className="text-xs font-black text-white block">{t('subscribeToAlerts', lang) || 'Subscribe to Route Alerts'}</span>
                        <span className="text-[10px] text-gray-400 font-bold">{t('subscribeAlertsDesc', lang) || 'Get SMS notifications for your routes'}</span>
                    </div>
                </div>
                <button
                ...
                >
                    {t('manageInProfile', lang) || 'Manage in Profile →'}
                </button>
            </div>
        </div>
    );
